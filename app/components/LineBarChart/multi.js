...
class LineBarChart extends React.Component {
  ...
  _tickSize = Math.max(...this.props.data.map(datum => datum.values.length));

  state = {
    xScale: d3
      .scaleLinear()
      .domain(
        d3.extent(this.props.data.map(datum => datum.values.map(d => d.a))),
      )
    ...
    yScale: d3
      ...
      .domain([
        0,
        d3.max(this.props.data.map(datum => datum.values.map(d => d.b))),
      ])
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    ...
    xScale
      .domain(
        d3.extent(nextProps.data.map(datum => datum.values.map(d => d.a))),
      )
    ...
    yScale
      .domain([
        0,
        d3.max(nextProps.data.map(datum => datum.values.map(d => d.b))),
      ])
  ...
  render() {
    const { _innerWidth, _innerHeight, _tickSize } = this;
    ...
    console.log(data.map(datum => datum.values));
    ...
    return (
      <>
        ...
        <Svg width={width} height={height}>
          ...
          {data.map(datum => (
            <Line key={uid(datum)} generator={line} source={datum.values} />
          ))}
        </Svg>
      </>
    );
